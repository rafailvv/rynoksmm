...
async def get_smm_by_ta(t):
    ...
    for x in smm:                                                       # Идем по всем смм, у которых хотя бы какая-то ца есть в помеченных галочкой
        v = dict_of_smm.get(x[-1], list(x[:-1]) + [0])
        dict_of_smm[x[-1]] = v[:-1] + [v[-1] + 1]                       # Если человек уже в словаре - добавляем к кол-ву его вхождений +1, если нету, создаем с 0 и делаем +1
    dict_of_smm = sorted(dict_of_smm.items(), key=lambda x: -x[1][-1])  # Сортируем по кол-ву вхождений - чем больше => больше всего совпадающих ца
    await disconnect_db(db, cur)
    return dict_of_smm
