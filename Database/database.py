...
async def get_smm_by_ta(t):
    ...
    tn = []
    for ta in t:
        tn.append(await get_ta_id(ta))
    t = tn
    if len(t) > 1:
        cur.execute(
            f"""
                        SELECT full_name, age, town, photo, cost, description, user_id
                        FROM smm 
                        INNER JOIN target_audience_smm
                        ON target_audience_smm.smm_id = smm.user_id
                        WHERE target_audience_id IN {tuple(t)} AND smm.date_sub > '{datetime.utcnow()}'
                    """
        )
    elif len(t) == 1:
        cur.execute(
            f"""
                        ...
                        INNER JOIN target_audience_smm
                        ON target_audience_smm.smm_id = smm.user_id
                        WHERE target_audience_id = '{t[0]}' AND smm.date_sub > '{datetime.utcnow()}'
                    """
        )
    smm = []
    try:
...
async def add_ta(id, t):
    ...
    for ta in t:
        ...
        cur.execute(
            f"""
                        INSERT INTO target_audience_smm(smm_id, target_audience_id) VALUES ({id}, {await get_ta_id(ta)})
                    """
        )
    db.commit()
    await disconnect_db(db, cur)
...
async def is_smm(user_id):
    ...
    cur.execute(f"""
                ...
                """)
    is_smm = cur.fetchone()[0]
    await disconnect_db(db, cur)
    return is_smm


async def get_date_sub(user_id):
    db, cur = await connect_db()
    cur.execute(f"""
                SELECT date_sub FROM smm WHERE user_id = {user_id}
                """)
    date_sub = cur.fetchone()[0]
    await disconnect_db(db, cur)
    return date_sub


async def add_date_sub(user_id, date_sub):
    db, cur = await connect_db()
    cur.execute(f"""
                UPDATE smm SET date_sub = '{date_sub}' WHERE user_id = {user_id}
                """)
    db.commit()
    await disconnect_db(db, cur)


async def get_ta_by_user_id(user_id):
    db, cur = await connect_db()
    cur.execute(f"""
                SELECT target_audience_id FROM target_audience_smm WHERE smm_id = {user_id}
                """)
    ta_id = cur.fetchall()
    await disconnect_db(db, cur)
    return ta_id
...
async def delete_smm(user_id):
    db, cur = await connect_db()
    cur.execute(f"""
                    DELETE FROM smm WHERE user_id = {user_id}
                """)
    db.commit()
    await disconnect_db(db, cur)
